...
describe('Process Lambda', () => {
  ...
  describe('', () => {
    ...
    beforeEach(() =>{
      ...
      sandbox.stub(notificationService, 'fetchFromAPI')
        .withArgs(sinon.match(process.env.TasksAPI)).callsFake(resolveTasks)
        .withArgs(sinon.match(process.env.UsersAPI)).callsFake(resolveUsers);
      sandbox.stub(notificationService, 'findByID')
        .withArgs( sinon.match.any, sinon.match('taskID'), sinon.match.any).callsFake(findTask)
        .withArgs( sinon.match.any, sinon.match('id'), sinon.match.any).callsFake(findUser);
